...
## 2. Correção da positivação — IMPLEMENTADA (não commitada)
...
### Implementação — FEITA (não commitada)
...
Tudo está **na working tree, sem commit**. Arquivos novos: `positivacao.py`, `recuperacao.py`,
...
⚠️ Localmente, rodar com `-p fastredis_plugin` (plugin fora do repo que troca o `_R_LOGIN` por
fakeredis); sem Redis local o login espera o timeout e a suíte leva horas em vez de 6 minutos.

### Para ir a produção (checklist)
- [ ] OK do Gabriel → commit numa branch `feat/**` → validar em `:teste` → merge na `main`.
